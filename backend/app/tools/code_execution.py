...
class CodeExecutionTool(BaseTool):
    ...
    BLOCKED_IMPORTS = {
        ...
        'sqlite3', 'dbm', 'gdbm', 'webbrowser', 'ctypes', 'mmap', 'resource',
        'signal', 'pty', 'tty', 'pickle', 'marshal', 'shelve', 'dbm',
        'multiprocessing', 'threading', 'concurrent', 'asyncio.subprocess'
    }
    
    BLOCKED_BUILTINS = {
        'open', 'exec', 'eval', 'compile', '__import__', 'input',
        'exit', 'quit', 'help', 'reload'
    }
            ...
            if isinstance(node, ast.Call):
                if isinstance(node.func, ast.Attribute):
                    if node.func.attr in ['read', 'write', 'open']:
                        return False, f"File operations are not allowed"
                ...
                if result.returncode == 0:
                    return ToolResult(
                        ...
                        data={
                            ...
                            "execution_time": f"{timeout}s limit"
                        }
                    )
                else:
                    return ToolResult(
