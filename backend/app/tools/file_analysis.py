...
class FileReadTool(BaseTool):
    ...
    async def execute(self, filepath: str, limit: int = 100, offset: int = 0) -> ToolResult:
        ...
        try:
            ...
            if not path.is_file():
                return ToolResult(success=False, data=None, error=f"Path is not a file: {filepath}")
    ...
    async def execute(self, path: str = ".", recursive: bool = False) -> ToolResult:
        ...
        try:
            ...
            if not target_path.is_dir():
                return ToolResult(
                    success=False, data=None, error=f"Path is not a directory: {path}"
                )
            ...
            return ToolResult(success=False, data=None, error=f"Failed to list files: {str(e)}")
...
class FileInfoTool(BaseTool):
    ...
    async def execute(self, filepath: str) -> ToolResult:
        ...
        try:
            ...
            if not path.exists():
                return ToolResult(success=False, data=None, error=f"Path not found: {filepath}")
            ...
            return ToolResult(success=False, data=None, error=f"Failed to get file info: {str(e)}")
