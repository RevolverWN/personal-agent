...
async def stream_chat_response(
    ...
    async for chunk in agent_core.chat_stream(
        message=message, model=model, temperature=temperature, history=history
    ):
        full_content += chunk
        data = {"type": "content", "content": chunk, "conversation_id": conversation_id}
...
async def chat(
    ...
    # Save user message
    user_message = Message(conversation_id=conversation_id, role="user", content=request.message)
    db.add(user_message)
    await db.commit()
    ...
    return {
        "message": ChatMessage(
            role="assistant", content=response_data["content"], timestamp=datetime.now(UTC)
        ),
        "conversation_id": conversation_id,
        "model": model,
...
async def execute_tool(
    tool_name: str, arguments: dict, current_user: User = Depends(get_current_user)
):
