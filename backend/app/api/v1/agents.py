...
async def list_builtin_roles(current_user: User = Depends(get_current_user)):
...
async def list_agents(
    ...
    agents = await manager.get_user_agents(user_id=current_user.id, include_builtin=include_builtin)
    return agents
...
async def create_agent_from_role(
    ...
    agent = await manager.create_agent_from_role(
        user_id=current_user.id, role_id=role_id, custom_name=custom_name
    )
