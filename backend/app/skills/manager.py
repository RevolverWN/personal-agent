...
class SkillManager:
    ...
    async def execute_skill(
        ...
        if not skill.is_initialized():
            ...
            if not success:
                return SkillResult(
                    success=False, data=None, error=f"Failed to initialize skill '{skill_name}'"
                )

        try:
            return await skill.execute(action, params)
        except Exception as e:
            return SkillResult(success=False, data=None, error=f"Skill execution error: {str(e)}")
    ...
    def find_skill_for_action(self, action_name: str) -> BaseSkill | None:
